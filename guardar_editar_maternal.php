...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "SELECT curp_documento, certificado_medico, tutor_curp FROM Maternal WHERE id = ?";
    ...
    if (isset($_FILES['certificado_medico_pdf']) && $_FILES['certificado_medico_pdf']['error'] === UPLOAD_ERR_OK) {
        ...
        $certificado_medico = 'uploads/maternal/' . uniqid() . '_' . basename($_FILES['certificado_medico_pdf']['name']);
    ...
    if (isset($_FILES['curp_tutor_pdf']) && $_FILES['curp_tutor_pdf']['error'] === UPLOAD_ERR_OK) {
        ...
        $curp_tutor_pdf = 'uploads/maternal/' . uniqid() . '_' . basename($_FILES['curp_tutor_pdf']['name']);
        move_uploaded_file($_FILES['curp_tutor_pdf']['tmp_name'], $curp_tutor_pdf);
    } else {
        $curp_tutor_pdf = $curp_tutor_pdf_old;
    }

    $sql = "UPDATE Maternal SET nombre = ?, apellido_paterno = ?, apellido_materno = ?, edad = ?, curp_documento = ?, certificado_medico = ?, tutor_nombre = ?, tutor_apellido_paterno = ?, tutor_apellido_materno = ?, tutor_curp = ?, tutor_telefono = ?, direccion = ? WHERE id = ?";
    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            header("Location: expediente_maternal.php?message=Alumno editado con éxito");
        } else {
