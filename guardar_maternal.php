...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['certificado_medico']) && $_FILES['certificado_medico']['error'] === UPLOAD_ERR_OK) {
        $certificado_medico = 'uploads/maternal/' . uniqid() . '_' . basename($_FILES['certificado_medico']['name']);
        move_uploaded_file($_FILES['certificado_medico']['tmp_name'], $certificado_medico);
    }

    if (isset($_FILES['fotografia']) && $_FILES['fotografia']['error'] === UPLOAD_ERR_OK) {
        $fotografia = 'uploads/maternal/' . uniqid() . '_' . basename($_FILES['fotografia']['name']);
    ...
    $sql = "INSERT INTO Maternal (nombre, apellido_paterno, apellido_materno, edad, curp_documento, certificado_medico, fotografia, tutor_nombre, tutor_apellido_paterno, tutor_apellido_materno, tutor_curp, tutor_telefono, direccion, activo) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)";
